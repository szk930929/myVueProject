...
export default {
  ...
  data () {
  	return {
  		buyNum:1,
  		buyType:{label: '入门版', value: 0},
  		versions:[{ label: '客户版', value: 0 }],
  		period:{label: '半年',value: 0},
  		price:400,
  ...
  methods:{
   ...
   getPrice () {
   	...
   	for(var i = 0;i<this.versions.length;i++){
   		if(this.versions[i].value === 0){
   			vprice += 200;
   		}else if(this.versions[i].value === 1){
   			vprice += 300;
   		}else{
   			vprice += 500;
   		}
   	}
   		this.price = this.buyNum * (pprice + vprice);
   }
  }
}
</script>
